...
## Unstructured Data in Company Filings

Companay filings (e.g., 10-K) contain a large amount of unstructured text data, especially in sections such as the business description, risk factors, legal proceedings, and management’s discussion and analysis. Unlike structured financial statement data, this narrative information is not arranged in fixed rows and columns, which makes it more difficult to analyze using traditional spreadsheet methods. However, it is often rich in useful signals about firm strategy, uncertainty, governance, litigation exposure, and future expectations. `edgartools` can be used to extract unstructured data as well. 

```{python}
from edgar import *
set_identity("Sharif Islam mdshariful.islam@siu.edu")
filings = get_filings(form='10-K', amendments=False, filing_date="2026-01-05:2026-01-12")
filings_df = filings.to_pandas()

# Create a list to store the Item 1a and Item 7 text
items_texts = []
...
If you want to extract some specific companies' unstructured data, you can do so as well. 
...
for ticker in tickers:
    filings = company.get_filings(form="10-K", date=date_range,amendments=False)
    for n, filing in enumerate(filings):
        url = filing.url
        cik = filing.cik
        filing_date = filing.header.filing_date,
        reporting_date = filing.header.period_of_report,
        comn = filing.company,
        tic = Company(filing.cik).tickers
...
# Create a DataFrame from the Item 1c text data
company_items_df = pd.DataFrame(items_texts)
```

If you want to extract some accounting policies of some specific companies, you can do so using `edgartools` python package. 
...
for ticker in tickers:
    ...
    for filing in filings:
        ...
        try:
            ...
            acct_policy = xbrl.get_statement("accounting_policies")
            ...
            records.append({
                ...
                "accounting_policies": acct_policy,
            })
            ...
            records.append({
                ...
                "report_date": filing.report_date,
                #"fiscal_year": filing.report_date.year,
                "accounting_policies": None,
                "error": str(e),
            })

accounting_policy_df = pd.DataFrame(records)
```
...
$$
\text{r}_t = {P}_t/{P}_{t-1}-1
$$
where:

- $\text{P}_t$ = Adjusted Price at the End of Day $t$
